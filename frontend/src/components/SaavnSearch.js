...
const SaavnSearch = ({ socket, setCurrentSong }) => {
  ...
  return (
    <div className="p-4 max-w-md mx-auto text-white bg-gray-900 rounded-lg shadow-lg">
      <h2 className="text-lg font-semibold mb-2">Search Songs</h2>
      <div className="flex">
        <input
          type="text"
          ...
          onChange={(e) => setQuery(e.target.value)}
        />
        <button onClick={handleSearch} className="p-2 bg-blue-500 rounded-r">
          Search
        </button>
      </div>
      {loading && <p>Loading...</p>}
      ...
      <ul className="mt-4">
        {songs.map((song) => (
          <li key={song.id} className="flex justify-between p-2 border-b">
            <span>{song.name}</span>
            <button
              onClick={() => clickPlay(song.downloadUrl[2].url)}
